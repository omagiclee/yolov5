...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if cache_images:
            ...
            pbar.close()
        self.roi_ims = [None] * n
        self.roi_npy_files = [Path(f[:-4]+'_roi.npy') for f in self.img_files]
        if cache_images:
            gb = 0
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        desc = f"{prefix}Scanning '{path.parent / path.stverify_image_labelem}' images and labels..."
        with Pool(NUM_THREADS) as pool:
            pbar = tqdm(pool.imap(, zip(self.img_files, self.label_files, repeat(prefix))),
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            img, (h0,w0), (h, w) = self.load_image(index) #960 600
            img, ratio, pad = letterbox(img, (608, 960), auto=False, scaleup=self.augment) #960 608
    ...
    def load_image(self, i):
        ...
        if im is None:  # not cached in RAM
            ...
            if npy and npy.exists():  # load npy
                ...
                f = self.img_files[i]
                im = cv2.imread(f)  # BGR#1920 1200
                assert im is not None, f'Image Not Found {f}'
            h0, w0 = im.shape[:2]  # orig hw
            r = self.img_size / max(h0, w0)  # ratio# 960 /1920 = 0.5
    ...
    def load_roi_image(self, i):
        roi_im, f, fn = self.roi_ims[i], self.img_files[i], self.roi_npy_files[i]
        if roi_im is None:
            if fn.exists():
                im = np.load(fn)
            else:
